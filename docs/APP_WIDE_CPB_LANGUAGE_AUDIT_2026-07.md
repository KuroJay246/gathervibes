...
Specialized wording remains allowed only when an event category or capability makes it relevant, or when the organizer is viewing protected historical/reconciliation evidence.
...
| CPB financial evidence and baker evidence | `/dashboard` completed event view, `/payments`, `/operations`, `/event-review` | Historical evidence panels | `src/pages/DashboardPage.jsx`, `src/pages/PaymentsPage.jsx`, `src/pages/OperationsPage.jsx`, `src/pages/EventReviewPage.jsx`, `src/utils/financialEvidenceAudit.js` | Protected historical CPB evidence | Yes | Remains conditional on `getEventFinancialEvidenceAudit(eventId)`, which only returns CPB evidence. |
| CPB reconciliation preview | `/payments/reconciliation` | Locked read-only reconciliation tool | `src/pages/PaymentReconciliationPage.jsx`, `src/utils/paymentReconciliation.js` | Protected historical CPB evidence | Yes | Remains locked to explicit CPB confirmation text. |
...
Normal tutorial and ordinary daily workflow copy no longer teaches from CPB or CODEX_TEST. CPB evidence remains protected in historical, reconciliation, Reports, Operations, and System QA contexts. CODEX_TEST remains QA-only.
