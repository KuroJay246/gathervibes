...
| Tutorial routes | Tasks and Reconciliation were not standalone tutorial stops. | Organizer | Tutorial did not teach all current primary workflow checkpoints. | Retarget two lessons to `/tasks` and `/payments/reconciliation`. | 20 anchored lessons, route paths, and zero-business-write behavior remain. |
