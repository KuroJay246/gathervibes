...
- Web App Check initialization is supported by `src/lib/firebase.js` through `VITE_FIREBASE_APP_CHECK_SITE_KEY`; the production web app is registered with reCAPTCHA Enterprise and the public site-key configuration is deployed. Authenticated production runtime initialization is verified in monitoring mode. Enforcement remains disabled pending separate owner approval.
- Native Android App Check is implemented in `apps/mobile/src/lib/firebase.js` with provider selection from `apps/mobile/src/lib/appCheckConfig.js`. Emulator/development builds use the debug provider, production builds use Play Integrity, and token auto-refresh is enabled.
...
- Owner MFA was completed on 2026-08-26, restoring Firebase Console access to project `gathervibeshub`. In that restored session, the Android app `com.gathervibeshub.staff` was registered for App Check with Play Integrity, and the web app was registered with reCAPTCHA Enterprise. The web site-key configuration is deployed and authenticated monitoring-mode initialization is verified. Enforcement remains off; API-key restrictions, billing, backup/restore, monitoring receipt, and provider receipts remain unverified.
- The Android sign-out path now clears both provider and local sign-in errors. A freshly rebuilt debug APK was installed on `gsv_api36_staff` on 2026-08-26, and the guarded emulator regression passed 11/11 including explicit stale-error absence after sign-out.
- The current local `release` build type still uses the Android debug keystore in `apps/mobile/android/app/build.gradle`. This means the present SHA-1/SHA-256 used for local release-like artifacts are debug-certificate values and must not be treated as final production signing.
