...
Use this guide to demonstrate the organizer workflow safely without touching CPB production data.

## Sign in

1. Open the app and sign in with an approved organizer account.
2. Confirm the app opens to the organizer workspace.
3. If the session is stale, sign out and sign in again before the demo starts.

## Select the safe demo event

1. Open `Overview`, `Events`, or `System QA`.
2. Use the clearly labelled `Open Demo Event` or `Use CODEX_TEST` action.
...
- Prefix temporary demo business records with `QA_PHASE23S_`.
...
1. Delete the temporary demo registrations, tickets, and Operations entries created for the walkthrough.
2. Keep audit logs intact. Cleanup removes business records, not audit history.
3. Clear or change the Working Event after the demo if needed.

## Do not edit in CPB

- Do not create, edit, or delete demo records in CPB.
...
Before calling the demo build release-ready, complete:
