...
test('Google and email sign-in both retain admin allowlist verification', async () => {
  ...
  assert.match(authFlow, /shouldPreferRedirectGoogleSignIn/)
  assert.match(authFlow, /sanitizeReturnPath/)
  assert.match(firebaseConfig, /function resolveAuthDomain\(\)/)
  assert.match(firebaseConfig, /currentHost\.endsWith\('\.web\.app'\)/)
  assert.match(firebaseConfig, /currentHost\.endsWith\('\.firebaseapp\.com'\)/)
  assert.match(firebaseConfig, /authDomain: resolveAuthDomain\(\)/)
