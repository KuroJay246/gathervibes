...
test('Payments workspace classifies registration payment follow-up without inventing money', () => {
  const registrations = [
    { registrationId: 'paid', fullName: 'Paid Guest', personsAttending: 1, ticketPrice: 50, amountPaid: 50, paymentStatus: 'paid', paymentReference: 'FP-1' },
  ...
  assert.equal(workspace.summary.needsFollowUpCount, 3)
  assert.equal(workspace.filterCounts['finance-review'], 2)
...
test('Registration finance classification catches data defects and duplicate references', () => {
  ...
  assert.equal(classified.statusGroup, 'needs-review')
  assert.ok(classified.warnings.some((warning) => warning.code === 'overpaid'))
  assert.ok(classified.warnings.some((warning) => warning.code === 'balance-mismatch'))
...
test('Operations page and helpers keep registration payments separate from ledger totals', async () => {
  ...
  assert.match(operationsPage, /Do not add this automatically to registration payment totals/)
  ...
  assert.match(operationsPage, /Operations Net Position/)
  assert.match(operationsPage, /Visible Operations Net Position/)
