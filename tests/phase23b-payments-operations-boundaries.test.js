...
test('Payments route is organizer-facing while scanner navigation remains isolated', async () => {
  ...
  assert.match(shell, /Guests[\s\S]*\/registrations[\s\S]*\/payments[\s\S]*\/tickets[\s\S]*\/check-in/)
  assert.match(shell, /Operations[\s\S]*\/operations[\s\S]*\/event-review[\s\S]*\/payments\/reconciliation/)
