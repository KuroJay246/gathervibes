...
test('Phase 14B UI and rules expose required safe surfaces', async () => {
  ...
  assert.doesNotMatch(importsPage, /CPB Payment Audit Backfill|PaymentAuditBackfillPanel|cpb-audit-preview/)
  assert.match(registrationsPage, /Finance Review|Showing all registrations|Group of/)
  assert.doesNotMatch(registrationsPage, /regs \//)
  assert.match(ticketsPage, /Advanced Filters|Ticket Status|Payment Status|Check-In Status|Needs Review/)
  assert.match(checkInPage, /Check in selected|Undo check-in selected|Copy selected payment review list|Group Registrations|covering .* guest/)
  assert.match(checkInPage, /ConfirmDialog/)
  assert.doesNotMatch(checkInPage, /window\.confirm/)
  assert.doesNotMatch(checkInPage, /bulkDelete/)
  assert.match(templatesPanel, /What to leave blank|What not to put/)
  assert.doesNotMatch(templatesPanel, /CPB Payment Audit Backfill|Christina Morris|Cole also has/)
