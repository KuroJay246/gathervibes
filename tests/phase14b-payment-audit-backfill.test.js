...
test('Phase 14B UI and rules expose required safe surfaces', async () => {
  ...
  assert.match(registrationsPage, /Door Paid|To Pay at Door|Review Needed|Missing Ticket/)
  assert.match(ticketsPage, /Outstanding Balance|Not Checked In|Complimentary/)
  assert.match(checkInPage, /Check in selected|Undo check-in selected|Copy selected door list/)
  ...
  assert.match(operationsPage, /Event Operations \/ Money Tracker|operationsLedger|cancelLedgerEntry/)
