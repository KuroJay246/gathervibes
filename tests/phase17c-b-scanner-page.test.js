...
test('Phase 17C-B scanner route is outside AppShell and auto-selects a single assignment', async () => {
  ...
  assert.match(shell, /to="\/dashboard"[\s\S]*aria-label="Go to Home"/)
  assert.doesNotMatch(scannerPage, /to="\/dashboard"|aria-label="Go to Home"/)
})
...
test('Phase 17C-B scanner page is narrow and excludes admin-only controls', async () => {
  ...
  assert.match(scannerPage, /No new check-in write happened/)
  assert.match(scannerPage, /No offline writes are supported/)
  assert.match(scannerPage, /Scan next guest/)
  assert.match(scannerPage, /Next guest/)
  assert.match(scannerPage, /admin handles the existing admin-only undo path/i)
  assert.match(scannerPage, /if \(\/No ticket code\/i\.test\(warning\)\) return 'blush'/)
  assert.match(scannerPage, /if \(\/outstanding\|door payment\|not marked paid\/i\.test\(warning\)\) return 'gold'/)
  ...
  assert.match(scannerPage, /ConfirmDialog/)
  assert.doesNotMatch(scannerPage, /window\.confirm/)
...
test('Phase 17C-B1b Settings uses accessible query-linked tabs', async () => {
  const settings = await readFile('src/pages/SettingsPage.jsx', 'utf8')
  const pageTabs = await readFile('src/components/ui/PageTabs.jsx', 'utf8')

  assert.match(settings, /useSearchParams/)
  assert.match(settings, /const SETTINGS_TABS/)
  assert.match(settings, /PageTabs/)
  assert.match(settings, /SETTINGS_TAB_ALIASES/)
  assert.match(settings, /const requestedTab = SETTINGS_TAB_ALIASES\[searchParams\.get\('tab'\)\] \|\| searchParams\.get\('tab'\) \|\| 'access'/)
  assert.match(settings, /setSearchParams\(\(current\) =>/)
  assert.match(pageTabs, /role="tablist"/)
  assert.match(pageTabs, /role="tab"/)
  assert.match(pageTabs, /aria-selected=\{selected\}/)
  assert.match(pageTabs, /tabIndex=\{selected \? 0 : -1\}/)
...
test('Phase 17C-B3 AuthProvider checks staff path before not-approved error', async () => {
  ...
  assert.match(loginPage, /result\?\.workspaceDefaultRoute/)
  assert.match(loginPage, /navigate\(sanitizeReturnPath\(result\.workspaceDefaultRoute/)
...
test('Phase 17C-B scanner lookup preserves QR privacy and manual fallback', async () => {
  ...
  assert.match(scannerPanel, /Sound feedback/)
  assert.match(scannerPanel, /Vibration \/ haptic/)
  assert.match(scannerPanel, /no new check-in write happened/)
