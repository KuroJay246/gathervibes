...
test('Settings, QA, and Communications pages expose practical admin polish', async () => {
  ...
  assert.match(settings, /Your organizer account/)
  assert.match(settings, /photoURL/)
  assert.match(settings, /Protected owner and approved organizers/)
  assert.match(settings, /Public access/)
  assert.match(qa, /Run Read-Only Checks/)
  assert.match(qa, /Copy Technical Report/)
...
test('old internal labels are removed from organizer-facing UI', async () => {
  ...
  assert.match(settings, /Data & Messages/)
