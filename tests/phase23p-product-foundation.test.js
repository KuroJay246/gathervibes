...
test('Phase 23P adds permanent product QA and audit commands without production credentials', async () => {
  ...
  assert.match(packageJson.scripts['product:qa:emulator-checks'], /firestore-checkin-rules/)
  assert.match(packageJson.scripts['product:qa:emulator-checks'], /e2e:smoke:inner/)
  assert.match(packageJson.scripts['e2e:smoke'], /e2e:smoke:inner/)
  assert.match(runner, /doctor:changed/)
  assert.match(runner, /doctor:json/)
  assert.doesNotMatch(runner, /npm run e2e:smoke/)
