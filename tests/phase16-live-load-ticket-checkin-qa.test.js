...
test('Phase 16 keeps Firebase Hosting SPA routes and non-overblocking security headers', async () => {
  ...
  assert.match(valuesByKey.get('Content-Security-Policy'), /default-src 'self'/)
  assert.match(valuesByKey.get('Content-Security-Policy'), /frame-ancestors 'none'/)
  assert.match(valuesByKey.get('Content-Security-Policy'), /script-src 'self'/)
  assert.match(valuesByKey.get('Content-Security-Policy'), /style-src 'self' 'unsafe-inline'/)
...
test('root render has a safe browser loading fallback', async () => {
  ...
  assert.match(boundary, /categorizeAppError/)
  assert.match(boundary, /Failed to fetch dynamically imported module/i)
  assert.match(boundary, /browser may be holding an older page/)
  assert.match(boundary, /Try Again/)
  assert.match(boundary, /Reload Latest Version/)
  assert.match(boundary, /Check Connection/)
  assert.match(boundary, /Sign In Again/)
  ...
  assert.match(boundary, /firebaseProjectId/)
  assert.match(boundary, /Sign-in state/)
