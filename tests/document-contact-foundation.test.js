...
test('document helpers validate URL, categories, statuses, required state, expiry, and task prefill', () => {
  ...
  assert.equal(effectiveDocumentStatus(record, today), 'Expiring Soon')
  assert.equal(filterDocuments(documents, 'Missing / Needed').length, 1)
  assert.equal(filterDocuments(documents, 'Expired', '', '', today).length, 1)
  assert.deepEqual(buildDocumentSummary(documents, today), { total: 3, required: 2, missingRequired: 1, expiringSoon: 1, expired: 1 })
