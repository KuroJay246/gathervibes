...
test('17F status copy keeps 17E-C/D/E read-only and records current-head Daily QA success as non-blocking', async () => {
  ...
  assert.match(settings, /No active implementation phase on main/)
  assert.match(settings, /Phase 17G-A closed after organizer review PASS \/ approval package accepted \/ no Firestore rules deploy \/ no Firestore index deploy \/ no live workflow/)
  ...
  assert.match(health, /Phase 17G-A closed after organizer review PASS with no rules or index deploy/)
