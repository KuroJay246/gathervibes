...
test('17F status copy keeps 17E-C/D/E read-only and records current-head Daily QA success as non-blocking', async () => {
  ...
  assert.match(settings, /Phase 17G-B Firestore Rules Deployment Approval \+ Dry-Run Final Review only/)
  assert.match(settings, /Active on this review branch \/ no merge \/ no Firestore rules deploy \/ no Firestore index deploy \/ no live workflow change/)
  assert.match(settings, /Phase 17G-B2 Explicit Firestore Rules Deploy Authorization \+ Real Deploy \+ Immediate Smoke only/)
  ...
  assert.match(health, /Phase 17G-A closed after organizer review PASS with no rules or index deploy, and Phase 17G-B remains dry-run review only with no new deploy\./)
