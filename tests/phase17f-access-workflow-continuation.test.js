...
test('17F status copy keeps 17E-C/D/E read-only and records current-head Daily QA success as non-blocking', async () => {
  ...
  assert.match(settings, /Organizer review for 17E-C \/ 17E-D \/ 17E-E \/ 17F-A \/ 17F-B \/ 17F-C/)
  ...
  assert.match(readme, /Phase 17F-A is now active on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow Implementation Plan only\./)
  assert.match(readme, /Phase 17F-C is now active on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow manual smoke checklist only\./)
  assert.match(handoff, /Phase 17F-B is now active on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci` as the disabled Access Request service contract only\./)
