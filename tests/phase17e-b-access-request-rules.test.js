...
test('Phase 17E-B docs and current UI keep accessRequests non-live', async () => {
  ...
  assert.match(legacyReadme, /Phase 17F-B is now closed, merged-ready, and organizer accepted on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci` as the disabled Access Request service contract only\./)
  assert.match(legacyHandoff, /Phase 17F-C is now closed, merged-ready, and organizer accepted on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow manual smoke checklist only\./)
  assert.match(legacyReadme, /Phase 17E-B is now closed, merged-ready, and accepted by organizer review PASS on branch `codex\/phase-17e-b-access-request-rules-prototype-tests` at commit `dc108e18a5c5efd9cd3c283daeaeed1d440a45d9` as Access Request Rules Prototype \+ Tests, dry-run only\./)
  assert.match(legacyReadme, /Requesters cannot self-approve, cannot self-revoke, cannot create `staffProfiles`, cannot create `staffAssignments`, and cannot modify `approvedEmails`\./)
  assert.match(legacyReadme, /Phase 17E-E is now closed, merged-ready, and organizer accepted on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci` as the access workflow deployment readiness package and rollback plan only\./)
  assert.match(legacyHandoff, /Phase 17E-B closed as Access Request Rules Prototype \+ Tests after organizer review PASS, dry-run only/)
  assert.match(legacyHandoff, /Phase 17E-C is now closed, merged-ready, and organizer accepted on branch `codex\/phase-17e-cde-access-requests-ui-readiness-ci`\./)
  assert.match(readiness, /Phase 17E-E is deployment-readiness and rollback planning only\./)
  assert.match(legacyReadme, /Phase 17G-B is now closed, merged-ready, and organizer accepted on branch `codex\/phase-17g-b-firestore-rules-deployment-final-review` at commit `c998700d7882c3c5feaa52f59e9f21fd57c72b10` as Firestore Rules Deployment Approval \+ Dry-Run Final Review only\./)
  assert.match(legacyHandoff, /Phase 17G-B is now closed, merged-ready, and organizer accepted on branch `codex\/phase-17g-b-firestore-rules-deployment-final-review` at commit `c998700d7882c3c5feaa52f59e9f21fd57c72b10` as Firestore Rules Deployment Approval \+ Dry-Run Final Review only\./)
  assert.match(legacyReadme, /Phase 17G-B2 is now closed on branch `codex\/phase-17g-b2-firestore-rules-real-deploy-immediate-smoke` from base\/main tip `24d66e9bb49db2d3fe4f1722439dafe3bbd626c7`\./)
  assert.match(legacyHandoff, /Phase 17G-B2 is now closed on branch `codex\/phase-17g-b2-firestore-rules-real-deploy-immediate-smoke` from base\/main tip `24d66e9bb49db2d3fe4f1722439dafe3bbd626c7`\./)
