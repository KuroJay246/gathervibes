...
test('Phase 17E-B docs and status copy describe accessRequests as a closed dry-run prototype only', async () => {
  ...
  assert.match(settings, /No active implementation phase selected/)
  assert.match(settings, /Phase 17E-B is closed after organizer prototype review PASS \/ accepted dry-run only/)
  ...
  assert.match(settings, /no live workflow exists and this phase intentionally exposes no live write action for accessRequests, staffProfiles, staffAssignments, approvedEmails, or auditLogs/i)
  assert.match(qa, /Phase 17E-B is closed after organizer prototype review PASS with no live workflow change/)
  assert.match(qa, /Phase 17E-B closed as a dry-run-only accessRequests prototype with Firestore rules still undeployed/)
  assert.match(health, /Phase 17E-B is closed after organizer prototype review PASS with no live workflow change/)
  assert.match(health, /Phase 17E-B closed as a dry-run-only accessRequests prototype with Firestore rules still undeployed/)
  ...
  assert.match(readme, /Phase 17E-B is now closed after organizer prototype review PASS as a dry-run-only `accessRequests\/\{requestId\}` rules prototype; Firestore rules remain undeployed and the workflow is not live\./)
  assert.match(handoff, /Phase 17E-B closed as Access Request Rules Prototype \+ Tests after organizer review PASS, dry-run only/)
  assert.match(handoff, /Phase 17E-B is now closed, merged-ready, and accepted by organizer review PASS on branch `codex\/phase-17e-b-access-request-rules-prototype-tests` at commit `dc108e18a5c5efd9cd3c283daeaeed1d440a45d9` as Access Request Rules Prototype \+ Tests, dry-run only\./)
})
