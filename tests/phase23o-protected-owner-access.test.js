...
test('Phase 23O Settings UI marks protected owner as not removable through organizer settings', async () => {
  ...
  assert.match(settings, /Protected owner access is pinned to the Firebase UID/)
  assert.match(settings, /Protected Owner · cannot be removed or disabled here/)
  assert.match(settings, /Secondary organizer access remains controlled/)
...
test('Phase 23O guardrails preserve QR privacy, CODEX_TEST isolation, CPB locks, and registration correction hold', async () => {
  ...
  const importSources = await source('src/utils/importSources.js')
  ...
  assert.match(qaPage, /Do not use CPB for QA/)
  assert.match(qaPage, /CPB apply remains locked/)
  ...
  assert.match(importSources, /Dry-run first; no CPB writes until approval/)
})
