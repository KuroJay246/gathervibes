...
Daily QA preflight was investigated before this batch. The scheduled failures on July 2, 2026 at 12:20:36 UTC and earlier were not caused by the GitHub Actions Node 20 deprecation warning; the failing step was the built-auth smoke check still expecting the old `Sign up with Google` text on commit `1e48154527e4700bfbed92a2b5148d17a45182c5`. Current GitHub `main` now points to `c0b96340c993618dea20e430de9004c1a7031543`, and this branch updates the read-only Daily QA workflow to match the current login copy: `Continue with Google`, `Sign in with email`, and `Sign in securely`.
