...
Phase 17E-C is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Requests read-only/admin UI foundation only. It adds an admin-visible model/status surface for future `accessRequests/{requestId}` records, review-state guidance, disabled admin action markers, and explicit no-write boundaries. No live approval, decline, revoke, staff-profile creation, or event assignment workflow is enabled.

Phase 17E-D is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the requester access-request form UX prototype only. The requester form preview is disabled, admin-visible only, non-public, and submits nothing. No Firestore write, service call, public route, Cloud Function, or sending workflow is enabled.

Phase 17E-E is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the access workflow deployment readiness package and rollback plan only. `PHASE_17E_E_ACCESS_WORKFLOW_DEPLOYMENT_READINESS.md` carries the gates, dry-run boundaries, organizer approval checklist, rollback steps, and explicit not-live statements. Firestore rules remain undeployed and Firestore indexes remain undeployed in this batch.

Phase 17F-A is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow Implementation Plan only. `PHASE_17F_A_ACCESS_WORKFLOW_IMPLEMENTATION_PLAN.md` documents future approval, decline, revoke, staff-profile, and staff-assignment sequences together with audit actions, security boundaries, CPB protection, CODEX_TEST-only smoke boundaries, and explicit no-live-workflow / no-rules-deploy / no-index-deploy statements.

Phase 17F-B is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the disabled Access Request service contract only. `src/services/accessRequestContract.js` defines the future request statuses, fields, action names, and non-live function signatures, but it is not wired into the live UI, does not import Firestore writes, and any write-implying function stays disabled with a clear not-live result.

Phase 17F-C is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow manual smoke checklist only. `PHASE_17F_C_ACCESS_WORKFLOW_SMOKE_CHECKLIST.md` covers admin, scanner, and requester-preview smoke, CODEX_TEST-only limits, CPB no-touch rules, rollback verification, audit-log append-only checks, and no-approvedEmails workaround checks.
