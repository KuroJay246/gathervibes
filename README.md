...
Phase 17D-B is now the active scanner-only implementation phase. It is limited to private `/scanner` event-day polish such as clearer success/duplicate/pending/no-ticket states, faster next-guest flow, mobile touch/readability improvements, optional browser-only sound/haptic feedback, and explicit no-offline-writes guidance. It does not broaden permissions, implement Access & Roles workflows, implement lead-scanner permissions, change Firestore rules, or change the QR payload.
...
No live approval/revoke workflow exists yet, and no live lead-scanner permission exists yet. The next recommended subphase is Phase 17D-B scanner day-of polish only. Phase 17D-C Access & Roles read-only/admin UI foundation remains a later follow-on phase.
