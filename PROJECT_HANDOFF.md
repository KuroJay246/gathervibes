...
Last updated: July 7, 2026 (Phase 17E-C active as Access Requests read-only/admin UI foundation only; Phase 17E-D active as requester access-request form UX prototype only; Phase 17E-E active as access workflow deployment readiness and rollback planning only; Phase 17F-A active as access workflow implementation planning only; Phase 17F-B active as a disabled access-request service contract only; Phase 17F-C active as an access workflow manual smoke checklist only; Daily QA latest current-head run succeeded and older GitHub UI failures may be stale; Phase 17E-B closed as Access Request Rules Prototype + Tests after organizer review PASS, dry-run only; Phase 17E-A closed as Access Workflow Rules + Data Model Review only after organizer artifact review PASS; Phase 17D-C closed and merged as the Access & Roles read-only/admin UI foundation completed with organizer admin review PASS and organizer scanner review PASS; Phase 17D-D closed and merged as access workflow/rules-readiness planning completed; Phase 17D-B closed as scanner day-of polish completed, merge-ready, and Hosting-deployed with organizer scanner smoke PASS and admin after-smoke PASS; Phase 17D-A closed and merged as an approved planning-only blueprint; Phase 17C-B closed and merged with Firestore rules deployed in B2, Hosting-deployed, organizer scanner smoke passed, and admin after-smoke passed; Phase 17C-A closed, merged, and Hosting-deployed; Phase 17B closed, merged, and Hosting-deployed; Phase 17A closed, merged, and deployed; Phase 16 closed, merged, and deployed; Phase 15B closed, merged, and deployed)

Phase 17E-C is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Requests read-only/admin UI foundation only. It adds an admin-visible request model surface for future `accessRequests/{requestId}` records, request status guidance for `pending`, `approved`, `declined`, and `revoked`, disabled review controls, and explicit no-write boundaries. No live approval, decline, revoke, staff-profile creation, or event assignment workflow is enabled.

Phase 17E-D is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the requester access-request form UX prototype only. The form preview is disabled, admin-visible only, non-public, and submits nothing. No Firestore write, service call, public route, sending workflow, Cloud Function, or permission broadening is enabled in this phase.

Phase 17E-E is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the access workflow deployment readiness package and rollback plan only. `PHASE_17E_E_ACCESS_WORKFLOW_DEPLOYMENT_READINESS.md` carries deployment gates, rules/index dry-run boundaries, organizer approval checkpoints, and rollback steps for access workflow, admin access, scanner access, CPB protection, and append-only audit logging. Firestore rules remain undeployed and Firestore indexes remain undeployed in this batch.

Phase 17F-A is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow Implementation Plan only. `PHASE_17F_A_ACCESS_WORKFLOW_IMPLEMENTATION_PLAN.md` documents the future approval, decline, revoke, staff-profile, and staff-assignment sequences together with audit actions, security boundaries, CPB protection, CODEX_TEST-only smoke scope, and explicit no-live-workflow / no-rules-deploy / no-index-deploy statements.

Phase 17F-B is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the disabled Access Request service contract only. `src/services/accessRequestContract.js` defines future statuses, fields, action names, and not-live function signatures, but it is not wired into the live UI, does not import Firestore writes, and keeps every write-implying path disabled.

Phase 17F-C is now active on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow manual smoke checklist only. `PHASE_17F_C_ACCESS_WORKFLOW_SMOKE_CHECKLIST.md` covers admin, scanner, and requester-preview smoke, CODEX_TEST-only usage, CPB no-touch rules, rollback verification, audit-log append-only checks, and no-approvedEmails workaround checks.
