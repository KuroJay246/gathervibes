...
- Firebase Console access is blocked by a mandatory MFA prompt for the currently signed-in Google account. App Check registration/enforcement, billing, backup, and monitoring settings therefore remain unverified. Billing currently reports disabled, and no custom Cloud Monitoring policies were listed.
...
- Native preparation is present for bundle ID `com.gathervibeshub.staff`, Firebase iOS/Android configs, camera permission text, secure storage, Crashlytics, deep link `gsvstaff`, and EAS profiles. Physical iOS signing/device acceptance and native App Check remain owner actions.
- The Android owner-closeout run exposed and source-fixed a stale sign-in error banner that survived sign-out. The existing installed APK was not rebuilt by request; the next approved native build must verify the fix visually.
