...
Current-truth override as of 2026-08-04: use `docs/GSV_MASTER_SYSTEM_REFERENCE.md` and `docs/GSV_REPOSITORY_AND_MAINTENANCE_MANIFEST.md` for the concise current system and repository state. This file remains at the root because future agents and tests reference it directly, but much of the body below is historical phase evidence. Current synthetic QA uses `CODEX_DEMO - Full System Walkthrough` (`codex_demo_full_system_walkthrough`). `CODEX_TEST Live Verification Event` is retired historical terminology except where compatibility tests or archived evidence intentionally preserve it. Cake Piknik Barbados is a normal completed real event, not a special QA target, and must not receive synthetic writes.

Last updated: July 8, 2026 (Phase 17G-B2 is closed on branch `codex/phase-17g-b2-firestore-rules-real-deploy-immediate-smoke`; Firestore rules deployed to `gathervibeshub`, Firestore indexes not deployed, backend `accessRequests/{requestId}` rules live, embedded/in-app browser login redirect compatibility fix deployed, admin smoke passed, scanner smoke passed, admin route sanity confirmed no admin lock into `/scanner`, and no live approval/decline/revoke/staff-assignment/lead-scanner workflow enabled; Phase 17G-B closed, merged-ready, and organizer accepted on branch `codex/phase-17g-b-firestore-rules-deployment-final-review` as Firestore Rules Deployment Approval + Dry-Run Final Review only; organizer review PASS recorded; no Firestore rules deployed, no Firestore indexes deployed, and no live workflow enabled in that phase; Phase 17G-A closed, merged-ready, and organizer accepted as a Live Workflow Go/No-Go Review + Rules Deployment Approval Package only; organizer review PASS recorded; no Firestore rules deployed, no Firestore indexes deployed, and no live workflow enabled in that phase; Phase 17E-C closed, merged-ready, and organizer accepted as Access Requests read-only/admin UI foundation only; Phase 17E-D closed, merged-ready, and organizer accepted as requester access-request form UX prototype only; Phase 17E-E closed, merged-ready, and organizer accepted as access workflow deployment readiness and rollback planning only; Phase 17F-A closed, merged-ready, and organizer accepted as access workflow implementation planning only; Phase 17F-B closed, merged-ready, and organizer accepted as a disabled access-request service contract only; Phase 17F-C closed, merged-ready, and organizer accepted as an access workflow manual smoke checklist only; Daily QA latest current-head run succeeded and older GitHub UI failures may be stale; Phase 17E-B closed as Access Request Rules Prototype + Tests after organizer review PASS, dry-run only; Phase 17E-A closed as Access Workflow Rules + Data Model Review only after organizer artifact review PASS; Phase 17D-C closed and merged as the Access & Roles read-only/admin UI foundation completed with organizer admin review PASS and organizer scanner review PASS; Phase 17D-D closed and merged as access workflow/rules-readiness planning completed; Phase 17D-B closed as scanner day-of polish completed, merge-ready, and Hosting-deployed with organizer scanner smoke PASS and admin after-smoke PASS; Phase 17D-A closed and merged as an approved planning-only blueprint; Phase 17C-B closed and merged with Firestore rules deployed in B2, Hosting-deployed, organizer scanner smoke passed, and admin after-smoke passed; Phase 17C-A closed, merged, and Hosting-deployed; Phase 17B closed, merged, and Hosting-deployed; Phase 17A closed, merged, and deployed; Phase 16 closed, merged, and deployed; Phase 15B closed, merged, and deployed)

Phase 17G-B2 is now closed on branch `codex/phase-17g-b2-firestore-rules-real-deploy-immediate-smoke` from base/main tip `24d66e9bb49db2d3fe4f1722439dafe3bbd626c7`. Firestore rules were deployed to project `gathervibeshub`, Firestore indexes were not deployed, backend `accessRequests/{requestId}` rules are now live, and Hosting was redeployed after a login redirect compatibility fix for the embedded/in-app browser. Approved admin Google sign-in now completes in the in-app browser and admin smoke passed for `/dashboard`, `/settings`, `/qa`, `/registrations`, `/tickets`, and `/check-in`. Admin route sanity confirmed the approved admin is not locked into `/scanner`; stale returnUrl or browser history can still return the embedded browser to `/scanner` without indicating an auth lockout. Scanner smoke passed on the live `/scanner` route with assigned-event-only scope, CODEX_TEST as the only visible event context, no CPB visibility, no admin navigation, no Settings access, no Access & Roles access, no Undo Check-In, and no Check Out. Access Requests remains read-only, requester submit remains disabled/not live, no approval/decline/revoke workflow is live, no staff assignment editing is live, no lead-scanner workflow is live, `approvedEmails` remains unchanged and admin-only, CPB remains untouched, QR payload remains `GSV:TICKET:{ticketCode}`, `xlsx` remains absent, and `read-excel-file` remains active.
...
Phase 17F-A is now closed, merged-ready, and organizer accepted on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the Access Workflow Implementation Plan only. `PHASE_17F_A_ACCESS_WORKFLOW_IMPLEMENTATION_PLAN.md` is preserved as the implementation-planning artifact together with audit actions, security boundaries, CPB protection, and CODEX_TEST-only smoke scope.

Phase 17F-B is now closed, merged-ready, and organizer accepted on branch `codex/phase-17e-cde-access-requests-ui-readiness-ci` as the disabled Access Request service contract only. `src/services/accessRequestContract.js` is preserved and remains disabled/not live, not wired into the live UI, does not import Firestore writes, and keeps every write-implying path disabled.
...
Daily QA preflight was investigated before this batch. The scheduled failure at run `28589520049` on July 2, 2026 at 12:20:36 UTC fetched main commit `1e48154527e4700bfbed92a2b5148d17a45182c5` and failed in the built-auth smoke check because the workflow still expected `Sign up with Google`. The Node 20 deprecation warning affecting `actions/checkout@v4` and `actions/setup-node@v4` was not the root cause. The latest current-head run, `28875120502`, succeeded for branch commit `ca93b260e4b8c9d3e80041f095029dde79bf6794` on July 7, 2026 after the workflow fix was updated to scan all built `dist/assets/*.js` bundles for the current login copy: `Continue with Google`, `Sign in with email`, and `Sign in securely`. If GitHub still shows an older failure badge for this branch, treat it as stale older-run UI confusion unless the current head fails again.
...
Phase 17E-A is now closed, merged-ready, and accepted by organizer artifact review PASS. Branch `codex/phase-17e-a-access-workflow-rules-data-model-review` at commit `cb0047f310f1bbf61515088328e7f65410d20400` preserved `PHASE_17E_A_ACCESS_RULES_DATA_MODEL_REVIEW.md` as the accepted artifact. This phase was limited to access-workflow rules review, staff profile and assignment model review, audit-log action review, and future index/readiness review only. It did not implement a live approval workflow, revoke workflow, staff assignment editing workflow, lead-scanner permission, Firestore rules deployment, or Firestore indexes deployment. It did not broaden permissions; `approvedEmails` remained unchanged and admin-only; scanner remained assigned-event-only with no undo/check-out; admin undo remained admin-only where implemented; CPB remained untouched; CODEX_TEST remained QA/smoke only; QR payload remained `GSV:TICKET:{ticketCode}`; `xlsx` remained absent; and `read-excel-file` remained active.
...
- Phase 17C-B: Firestore Rules Deployment Approval + Live Scanner/Staff Smoke + Scanner-Only PWA Mode — closed, merged, Firestore rules deployed in B2, Hosting-deployed, organizer scanner smoke passed, admin after-smoke passed.
- Phase 17D-A: Access & Roles Planning + Scanner Day-of Polish Blueprint — closed, merged, planning-only blueprint approved; no new live permissions or workflow changes.
- Phase 17D-C: Access & Roles Read-Only/Admin UI Foundation — closed, merged, organizer admin review PASS, organizer scanner review PASS, read-only only.
...
## 23. Phase 17E-B closeout and recommended next gate
...
Phase 17E-B closeout status:

- Phase 17E-B: Access Request Rules Prototype + Tests
- Closed and merged-ready after organizer review PASS
- `accessRequests/{requestId}` prototype accepted
- Requester cannot self-approve or self-revoke
- Requester cannot create `staffProfiles` or `staffAssignments`
- Requester cannot modify `approvedEmails`
- Admin-only review boundary accepted
- Accepted statuses: `pending`, `approved`, `declined`, `revoked`
...
- No Firestore indexes deploy unless separately approved
- No CPB QA
- No approvedEmails workaround

Recommended next phase after 17E-B closeout:

- Phase 17E-C: Access Requests read-only/admin UI foundation
- No live approval/revoke workflow yet
- No staff assignment editing yet
- No lead-scanner implementation yet
- No Firestore rules deploy unless separately approved
- No Firestore indexes deploy unless separately approved
- No CPB QA
- No approvedEmails workaround
