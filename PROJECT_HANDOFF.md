...
Daily QA preflight was investigated before this batch. The scheduled failure at run `28589520049` on July 2, 2026 at 12:20:36 UTC fetched main commit `1e48154527e4700bfbed92a2b5148d17a45182c5` and failed in the built-auth smoke check because the workflow still expected `Sign up with Google`. The Node 20 deprecation warning affecting `actions/checkout@v4` and `actions/setup-node@v4` was not the root cause. Current GitHub `main` now points to `c0b96340c993618dea20e430de9004c1a7031543`, and this branch updates the read-only workflow to match the current login copy: `Continue with Google`, `Sign in with email`, and `Sign in securely`.
