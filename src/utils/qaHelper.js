...
export const qaChecklist = [
  ...
  'Complete one check-in for a CODEX_TEST guest',
  'Confirm a duplicate check-in is blocked',
...
export function buildQaSampleCsv(prefix = buildQaTestPrefix()) {
  return [
    'Full name,Email,Phone,Group name,Persons attending,Payment status,Payment reference,Notes',
    `${prefix} Guest One,${prefix.toLowerCase()}_guest1@example.com,2465550101,${prefix} Group,1,complimentary,${prefix}-001,Manual QA import row for CODEX_TEST only`,
    `${prefix} Guest Two,${prefix.toLowerCase()}_guest2@example.com,2465550102,${prefix} Group,2,pending,${prefix}-002,XLSX or CSV QA row for CODEX_TEST only`,
  ].join('\n')
}
