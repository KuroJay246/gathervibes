...
const ACCESS_MODEL_SECTIONS = [
  {
    ...
    body: 'Phase 17E-C closed after organizer review PASS with an admin-visible, read-only foundation for future access requests. Phase 17G-A is closed after organizer review PASS as the accepted approval package only, and Phase 17G-B is the current final dry-run review only. This surface documents the request shape and does not approve, decline, revoke, or submit anything.',
    rows: [
  ...
  },
  {
    ...
    body: 'No Firestore rules deploy, no Firestore index deploy, no permission broadening, and no write mutations are introduced in Phase 17G-B. This review branch keeps the approval package closed, keeps every access-workflow path non-live, and performs dry-run review only.',
    rows: [
...
const ROADMAP_SECTIONS = [
  ...
  { title: '2. Current active phase', items: [
    ['Phase 17G-B Firestore Rules Deployment Approval + Dry-Run Final Review only', 'Active on this review branch / no merge / no Firestore rules deploy / no Firestore index deploy / no live workflow change'],
  ...
  { title: '3. Next recommended phase', items: [['Phase 17G-B2 Explicit Firestore Rules Deploy Authorization + Real Deploy + Immediate Smoke only', 'No automatic deploy / no index deploy unless separately approved / immediate smoke required after any future approved rules deploy']] },
