...
const ROADMAP_SECTIONS = [
  ...
  { title: '2. Current active phase', items: [
    ...
    ['Daily QA preflight', 'Scheduled failure traced to stale built-auth smoke text, not the Node 20 deprecation warning'],
  ] },
  { title: '3. Next recommended phase', items: [['Phase 17E-F rules deployment approval package only', 'Only after organizer review PASS for 17E-C / 17E-D / 17E-E']] },
    ...
    access: (
      <SettingsSection eyebrow="Access & Roles" title="Read-only admin foundation">
        <div className="rounded-2xl border border-[#E6D4B4] bg-[#FFF8EA] p-4 text-sm leading-6 text-[#715D46]">
          Approved-admin allowlist remains active owner/admin enforcement. Approved admin allowlist access remains admin-level only; approvedEmails remains admin-level access only. Do not add staff/scanners/helpers to approvedEmails. Temporary event-day helpers should not be added to approvedEmails. Phase 17D-B remains closed after scanner/admin smoke PASS. Phase 17D-C is closed and merged after organizer admin review PASS and organizer scanner review PASS. Phase 17D-D readiness planning is closed and merged as planning-only. Phase 17E-A is closed after organizer artifact review PASS as accepted rules/data-model review only. Phase 17E-B is closed after organizer prototype review PASS as an undeployed access-request rules prototype and test pass only. Phase 17E-C is active here as a read-only/admin UI foundation only. Phase 17E-D is active here as a disabled requester form preview only. Phase 17E-E remains readiness and rollback planning only. No live approval, decline, revoke, assignment editing, lead-scanner, or rules-deploy workflow is implemented here.
        </div>
        ...
        <div className="mt-5 rounded-2xl border border-[#EFE2DA] bg-[#FBF8F5] p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-[#8A7468]">Future Access & Roles workflow only: pending access requests, approve/decline staff access, assign role, assign event, revoke access, and set inactive/revoked status. Role editing remains deferred to a later approved workflow. Settings should not rewrite Firestore rules; do NOT rewrite Firestore rules from Settings UI. Phase 17E-B closed after organizer prototype review PASS and remains limited to an undeployed <code>accessRequests/&#123;requestId&#125;</code> rules prototype plus tests and status copy only. Phase 17E-C adds an admin-visible read-only request surface only, Phase 17E-D adds a disabled requester form preview only, and Phase 17E-E adds readiness/rollback planning only. Firestore rules stay stable in production and were not deployed for this batch; no live workflow exists and this batch intentionally exposes no live write action for accessRequests, staffProfiles, staffAssignments, approvedEmails, or auditLogs. <code>PHASE_17D_PLAN.md</code> remains the blueprint for 17D-A, <code>PHASE_17D_D_ACCESS_WORKFLOW_READINESS.md</code> remains the workflow-readiness document, <code>PHASE_17E_A_ACCESS_RULES_DATA_MODEL_REVIEW.md</code> is the accepted review artifact, and <code>PHASE_17E_E_ACCESS_WORKFLOW_DEPLOYMENT_READINESS.md</code> is the current deployment-readiness package.</p>
        </div>
