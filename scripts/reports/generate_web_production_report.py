...
OUT = EVIDENCE / 'reports' / 'GSV_Web_Production_Security_Reliability_and_Visual_Evidence_Report.pdf'
...
responsive = [[image('after/local-checkin-tablet.png', 2.55 * inch), para('<b>Tablet check-in</b><br/>Synthetic fixture showing event-day check-in, QR/manual lookup, readiness counts, and bottom navigation.', 'Caption')], [image('after/local-qa-mobile.png', 2.0 * inch), para('<b>Mobile System QA</b><br/>Synthetic fixture showing compact navigation, status checks, event scope, and safe QA guidance.', 'Caption')], [image('android-regression/staff-login-fresh.png', 2.35 * inch), para('<b>Android staff package</b><br/>Fresh launch on isolated gsv_api36_staff with com.gathervibeshub.staff installed. No mobile source/build change was made in this web release.', 'Caption')]]
...
for item in ['Register the production web App Check site key and enforce App Check after confirming approved origins.', 'Sign in as the Protected Owner on the live site and execute owner, organizer, staff assignment, cross-event denial, audit, ticket, check-in, and import acceptance using approved production test records.', 'Register provider OAuth callbacks, secrets, scopes, and webhooks; capture an authenticated receipt for each live provider.', 'Configure monitoring, billing budgets, quotas, abuse monitoring, scheduled backups, retention, and a documented restore rehearsal.', 'Complete iOS physical-device/signing acceptance and resolve or explicitly accept the existing mobile dependency-audit residual.']:
    story.append(para('• ' + item))
story += [para('Artifacts', 'Section'), para('The complete package is in output/web-production-completion/: the readiness ledger, before/after security header captures, browser screenshots, Android regression evidence, and this PDF report. Canonical handoff documents were updated to preserve the distinction between deployed behavior and owner-controlled release actions.')]
