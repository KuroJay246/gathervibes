...
responsive = [[image('after/local-checkin-tablet.png', 2.55 * inch), para('<b>Tablet check-in</b><br/>Synthetic fixture showing event-day check-in, QR/manual lookup, readiness counts, and bottom navigation.', 'Caption')], [image('after/local-qa-mobile.png', 2.0 * inch), para('<b>Mobile System QA</b><br/>Synthetic fixture showing compact navigation, status checks, event scope, and safe QA guidance.', 'Caption')], [image('owner-closeout/android-e2e-pass.png', 2.35 * inch), para('<b>Android regression result</b><br/>Fresh rebuilt staff APK evidence: guarded emulator regression passed 11/11, with explicit assertion that sign-out leaves no stale Sign-in failed banner. The image is redacted and contains no credentials or ticket code.', 'Caption')]]
