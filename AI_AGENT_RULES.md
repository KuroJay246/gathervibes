...
- Roadmap 7-20 modernization is ongoing. Feature-owned read models/contracts now exist under `src/features/` for Registrations, Dashboard, Check-In, Operations, and Imports, but the full roadmap is not complete until large-data, concurrent check-in, cost/listener, monitoring, backup/restore, billing/quota, provider, visual, PDF, and Git closeout evidence is current.
